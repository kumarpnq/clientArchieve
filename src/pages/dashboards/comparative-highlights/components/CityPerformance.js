...
function CityPerformance(props) {
  ...
  const { data, loading } = useChartAndGraphApi({
    ...
    category: City,
    path: `data.doc.Report.${City}.buckets`
  })
  ...
  return matches ? (
    <CityTable data={data} loading={loading} selectMediaType={selectMediaType} changeMediaType={changeMediaType} />
  ) : (
    <CityWidget data={data} loading={loading} selectMediaType={selectMediaType} changeMediaType={changeMediaType} />
  )
}
...
function CityTable(props) {
  ...
  useEffect(() => {
    ...
    const cities = selectedCategory !== -1 ? [data.at(selectedCategory)] : data

    // Process data with added safety checks
    cities.forEach((dataItem, index) => {
    ...
    <Stack direction='row' spacing={2}>
      {(data[selectedCategory] || selectedCategory === -1) && (
        <Button size='small' onClick={openMenu} endIcon={<KeyboardArrowDown />}>
          {data[selectedCategory]?.key ?? 'All Cities'}
        </Button>
      )}
