...
const SubjectVScore = props => {
  ...
  const journalists = JournalistVScoreData.map(i => i.reportingSubject)
  ...
  const handleMenuClick = menuItem => {
    switch (menuItem) {
      ...
      case 'table':
        ...
        break
      case 'image':
        toBlob(document.getElementById('subject-vScore')).then(function (blob) {
          const url = window.URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = 'subject-vScore.png'
      ...
      case 'xlsx':
        if (chartData) {
          ...
          XLSX.writeFile(wb, 'subject-vScore.xlsx')
        }
        break
      default:
        break
  ...
  return (
    <>
      ...
      <Card sx={{ height: '100%' }}>
        ...
        <CardContent onClick={handleChartClick} id='subject-vscore'>
          {loading ? (
      ...
      <AddScreen open={openAddPopup} setOpen={setOpenAddPopup} reportId={'subjectVscore'} path={path} />
    </>
  )
}
