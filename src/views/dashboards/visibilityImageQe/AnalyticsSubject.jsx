...
const AnalyticsSubject = props => {
  ...
  const vscore = dataForCharts.length > 0 ? dataForCharts.map(data => data.vScore) : []
  ...
  const handleMenuClick = menuItem => {
    switch (menuItem) {
      ...
      case 'table':
        ...
        break
      case 'image':
        toBlob(document.getElementById('subject-visibility')).then(function (blob) {
          const url = window.URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = 'subject-visibility.png'
      ...
      case 'pdf':
        ...
        toBlob(document.getElementById('subject-visibility')).then(function (blob) {
          ...
          img.onload = function () {
            ...
            doc.save('subject-visibility.pdf')
      ...
      case 'xlsx':
        if (chartData) {
          ...
          XLSX.writeFile(wb, 'subjectVisibility.xlsx')
        }
        break
      default:
        break
        ...
        <Table>
          ...
          <TableBody>
            {chartData.map((item, index) => (
              <TableRow key={index}>
                <TableCell size='small'>{item.reportingSubject}</TableCell>
            ...
            <Box>
              ...
              <Menu keepMounted anchorEl={anchorEl} onClose={handleClose} open={Boolean(anchorEl)}>
                {asPath === '/dashboards/custom/' ? (
                  <MenuItem onClick={() => handleRemoveFromChartList('Subject')}>Remove from Custom</MenuItem>
                ) : (
